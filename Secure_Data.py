...
if "store_data" not in st.session_state:
    st.session_state.store_data = load_stored_data()
...
def decrypt_data(encrypted_text, passkey, data_id):
    try:
        ...
        if data_id in st.session_state.store_data and st.session_state.store_data[data_id]["passkey"] == hashed_passkey:
...
if st.session_state.current_page == "Home":
    st.markdown(
        "### 🔐 Your personal, encrypted data locker — built with Python & Streamlit.")
    st.write(
        "Utilize this application to **safeguard and access data** employing distinct passkeys.")
    ...
    st.info(
        f"Currently storing {len(st.session_state.store_data)} encrypted data entries.")
    ...
    if st.button("Encrypt & Save"):
        st.text_input(
            "Passkey", help="Enter a strong password to encrypt your data.")
        if user_data and passkey and confirm_passkey:
            if passkey != confirm_passkey:
                ...
                st.session_state.store_data[data_id] = {
                    "encrypted_text": encrypted_text,
                    "passkey": hashed_passkey
                }
                
                # Save to file
                save_stored_data(st.session_state.store_data)
                ...
                st.info(
                    "⚠️ Save this Data ID! You'll need it to retrieve your data.")
        else:
    ...
    if st.button("Decrypt"):
        if data_id and passkey:
            if data_id in st.session_state.store_data:
                encrypted_text = st.session_state.store_data[data_id]["encrypted_text"]
    ...
    if time.time() - st.session_state.last_attempt_time < 10 and st.session_state.failed_attempts >= 3:
        remaining_time = int(
            10 - (time.time() - st.session_state.last_attempt_time))
        st.warning(
...
# Add a footer
st.markdown("---")
st.markdown("🔐 **HS Secure Vault** &copy; 2025 | Built with ❤️ using Streamlit")
st.markdown(
